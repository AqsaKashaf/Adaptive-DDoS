...
tcp_size = 0.000038
udp_size = 0.0624
dns_amp_size = amp_factor * udp_size
...
def randMixAttack(Process_Cap,Server_cap,Link_Cap,ISP_Cap,budget):
...
def randIngressAttack(Process_Cap,Server_cap,Link_Cap,ISP_Cap,budget):
    bits_to_attack = 0
    pc,sc,lc,isp_cap = Process_Cap,Server_cap,Link_Cap,ISP_Cap
    attack_vol = {'TCP':0,'UDP':0,'DNS':0}
    while((attack_vol['TCP'] + attack_vol['UDP'] + attack_vol['DNS'])<=budget):
...
if __name__ =="__main__":
    ...
    pc,sc,lc,isp_cap = Process_Queue,Backlog,Link_Queue,ISP_Queue
    bits_Mix, bits_Ing, bits_Q = 0,0,0
    
    start = time.time()
    for i in range(num_rounds):
        budget = random.unifom(1, attack_budget)
        ...
        if(attack_budget<=0):
            break
        print("Attack {}, PC {}, SC {}, LC {}, IC {}".format(i+1,pc,sc,lc,isp_cap))
    ...
    for i in range(num_rounds):
        ...
        attack_list =[attacker[0]*tcp_size,attacker[1]*udp_size,attacker[2]*dns_size]
        
        budget = random.uniform(1,attack_budget)
        attack_budget-= budget
        
        pc,sc,lc,isp_cap,bits=SmartAttack(attack_list,ingress,pc,sc,lc,isp_cap,budget)
        bits_Q+= bits
        end = time.time()
        for l in range(0,3):
            RTT[l]-=(end-start)
