...
def mergeTraffic(attack):
    ...
    for j in range(0,3):
        if(j is ingress):
            Traffic[j]={'SYN':attack[ingress][0]+benign_traffic['SYN']*tcp_size,'UDP':attack[ingress][1],'DNS':attack[ingress][2], 'Data':benign_traffic['DATA']*udp_size/3,'Background':background*udp_size/3}
        else:
